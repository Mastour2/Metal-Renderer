struct Query {
    ...
    func query<C1: Component, C2: Component, C3: Component, C4: Component>(
        _: C1.Type, _: C2.Type, _: C3.Type, _: C4.Type,
        excluding: [any Component.Type],
        _ logic: (Entity, C1, C2, C3, C4) -> Void
    ) {
        for entity in world.entities {
            guard let c1: C1 = world.fetch(C1.self, for: entity),
                let c2: C2 = world.fetch(C2.self, for: entity),
                let c3: C3 = world.fetch(C3.self, for: entity),
                let c4: C4 = world.fetch(C4.self, for: entity),

                !isExcluded(entity, excluding)
            else { continue }
            logic(entity, c1, c2, c3, c4)
        }
    }
